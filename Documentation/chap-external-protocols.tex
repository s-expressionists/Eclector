...
\Defgeneric {interpret-symbol} {client input-stream package-name \\
  symbol-name internp}

This generic function is called by the default method on
\texttt{interpret-symbol-token} to resolve a symbol name string and a
package name string to a representation of the designated symbol.  The
parameter \textit{input-stream} is the input stream from which
\textit{package-name} and \textit{symbol-name} were read.  The
parameter \textit{package-name} is a either a string designating the
package of that name, the keyword \texttt{:current} designating the
current package or the keyword \texttt{:keyword} designating the
keyword package.  The \textit{symbol-name} is the name of the desired
symbol.

The default method uses \texttt{cl:find-package} (or
\texttt{cl:*package*} when \textit{package-name} is \texttt{:current})
to resolve \textit{package-name} followed by \texttt{cl:find-symbol}
or \texttt{cl:intern}, depending on \textit{internp}, to resolve
\textit{symbol-name}.
